...
const ContributeModal: React.FC<Props> = ({
  ...
  const label = currency === tokens.cake ? t('Max. SLCT entry') : t('Max. token entry')
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max SLCT entry is capped by minimum between your average SLCT balance in the IFO SLCT pool, or the pool’s hard cap. To increase the max entry, Stake more SLCT into the IFO SLCT pool',
        )
      : t(
          'For the unlimited sale, Max SLCT entry is capped by your average SLCT balance in the IFO SLCT pool. To increase the max entry, Stake more SLCT into the IFO SLCT pool',
        ),
    {},
  )
      ...
      <ModalBody maxWidth="360px">
        ...
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough SLCT, you may not receive any IFO tokens at all and will only receive a full refund of your SLCT.',
            )}
            <Link
              fontSize="12px"
              display="inline"
              href="https://docs.silverlineswap.finance/products/ifo-initial-farm-offering"
              external
            >
              {t('Read more')}
            </Link>
