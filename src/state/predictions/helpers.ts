...
import { PredictionsClaimableResponse, PredictionsLedgerResponse, PredictionsRoundsResponse, PredictionsRoundIsRefundResponse } from 'utils/types'
import {
  BetResponse,
  getRoundBaseFields,
  getBetBaseFields,
...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    claimedSln: betResponse.claimedSln,
...
export const getRoundResult = (bet: Bet, currentEpoch: number): Result => {
  ...
  if (bet.round.position === BetPosition.TIE) {
    return Result.TIE
  }
...
export const getFilteredBets = (bets: Bet[], filter: HistoryFilter, currentEpoch: number) => {
  switch (filter) {
    case HistoryFilter.COLLECTEDBNB:
      return bets.filter((bet) => bet.claimed === true)
    case HistoryFilter.UNCOLLECTEDBNB:
      return bets.filter((bet) => {
        return !bet.claimed && (bet.position === bet.round.position || bet.round.failed === true)
      })
    case HistoryFilter.COLLECTEDSLN:
        return bets.filter((bet) => {
          return bet.claimedSln 
        })  
    case HistoryFilter.UNCOLLECTEDSLN:
      return bets.filter((bet) => {
        return !bet.claimedSln && bet.position !== bet.round.position && bet.round.failed !== true && !(bet.round.epoch >= currentEpoch - 1)
      })
...
export const getHasRoundFailed = (oracleCalled: boolean, closeTimestamp: number, bufferSeconds: number) => {
  if (!oracleCalled) {
    const closeTimestampMs = (closeTimestamp + bufferSeconds) * 1000
...
export const getHasRoundExpired = ( epochValidTill: number, closeTimestamp: number, bufferSeconds: number) => {
  if (closeTimestamp !== 0) {
    const closeTimestampMs = (closeTimestamp + bufferSeconds) * 1000
    if (Number.isFinite(closeTimestampMs)) {
      return Date.now() > epochValidTill || Date.now() < closeTimestampMs
    }
  }
...
export const getRefundable = async ( account: string,epochs: number[]): Promise<PredictionsRoundIsRefundResponse['refundableRound']> => {
  const address = getPredictionsAddress()
  const calls = epochs.map((epoch) => ({
    address,
    name: 'refundable',
  ...
  const refundableRoundResponse = await multicallv2<PredictionsRoundIsRefundResponse[]>(predictionsAbi, calls)
  
  return refundableRoundResponse.reduce((accum, refundableRound, index) => {
      const epoch = epochs[index]
      return {
        ...accum,
        [epoch]: refundableRound[0],
      }
    }, {})
}
...
export const serializePredictionsLedgerResponse = (ledgerResponse: PredictionsLedgerResponse): ReduxNodeLedger => ({
  position: ledgerResponse.position === 1 ? BetPosition.BULL : BetPosition.BEAR,
  amount: ledgerResponse.amount.toJSON(),
  claimed: ledgerResponse.claimed,
  SLNclaimed: ledgerResponse.SLNclaimed,
})
...
export const serializePredictionsRoundsResponse = (response: PredictionsRoundsResponse): ReduxNodeRound => {
  const {
    ...
    lockOracleId,
    closeOracleId,
    epoch_valid_till, //eslint-disable-line camelcase,
  } = response
  ...
  return {
    ...
    totalAmount: bullAmount.add(bearAmount).toJSON(),
    ...
    epoch_valid_till: epoch_valid_till.eq(0) ? null : epoch_valid_till.toNumber(),
    expired : getHasRoundExpired(epoch_valid_till.toNumber(), closeTimestamp.toNumber(), 60)
  }
}
